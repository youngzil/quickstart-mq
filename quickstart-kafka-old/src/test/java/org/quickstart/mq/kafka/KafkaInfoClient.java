package org.quickstart.mq.kafka;
...
public class KafkaInfoClient {
    ...
    // 首先获取到每个 topic 的所有分区 leader，然后使用 leader 的 host 创建一个 SimpleConsumer，请求 OffsetRequest
    ...
    private long getPartationOffset(SimpleConsumer consumer, String topic, int partition, long whichTime) {
        ...
        kafka.javaapi.OffsetRequest request =
            new kafka.javaapi.OffsetRequest(requestInfo, OffsetRequest.CurrentVersion(), consumer.clientId());
    ...
    private TreeMap<Integer, PartitionMetadata> findLeader(Map<String, Integer> bootstraps, String topic) {
        ...
        for (Map.Entry<String, Integer> bootstrap : bootstraps.entrySet()) {
            ...
            try {
                ...
                logger.error("Error communicating with Broker [{}] to find Leader for [{}] Reason: ", bootstrap, topic,
                    e);
            } finally {
                if (consumer != null) {
                    consumer.close();
